...
if ( $_SESSION[ 'loginin' ] == "1" )
  {
    ...
    if ( empty( $_GET[ 'type' ] ) || $_GET[ 'type' ] == "added" )
      {
        ...
        if ( $_GET[ 'sort_by' ] == "date1" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `friends` WHERE `id1` = '" . $id . "'" );
          } //$_GET[ 'sort_by' ] == "date1"
        elseif ( $_GET[ 'sort_by' ] == "date2" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `friends` WHERE `id1` = '" . $id . "' ORDER BY id DESC" );
          } //$_GET[ 'sort_by' ] == "date2"
        elseif ( empty( $_GET[ 'sort_by' ] ) || $_GET[ 'sort_by' ] == "random" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `friends` WHERE `id1` = '" . $id . "' ORDER BY RAND()" );
...
<div id="content-infoname"><b><?php
        echo '<a href="id' . $userrr[ 'id' ] . '">' . $userrr[ 'name' ] . ' ' . $userrr[ 'surname' ] . '</a>';
?> » Друзья</b><div style="float:right;"><a href="search.php?type=users&sort_by=random">Поиск пользователей (Добавить в друзья)</a></div></div>
...
<tr>
<td width="75" style="vertical-align:top;"><a href="http://l-lsoc.cf/id' . $user[ 'id' ] . '"><img src="' . $user[ 'avatar' ] . '" width="75" height="auto"></a></td>
...
<br><br>';
        ...
        if ( $user[ 'id' ] == "" )
          {
            echo '<div style="margin:0 -10px;padding:55px;"><center><img src="img/error.png"><br><br><b style="font-size:25px;">У Вас нет друзей</b><br><text style="font-size:14px;">Попробуйте найти себе друзей и повторить попытку</text></center></div>';
          } //$user[ 'id' ] == ""
?>
</div>
...
<div style="margin:10px;">
	<?php
        if ( $id == $_SESSION[ 'id' ] )
          {
?>
<a id="aprofile" href="friends.php?type=incoming&sort_by=random"><b>Заявки в друзья</b></a>
<a id="aprofile" href="friends.php?type=outcoming&sort_by=random">Ваши подписки</a>
<hr style="margin-left:-14px;margin-right:-20px;margin-top:10px;margin-bottom:10px;">
<?php
          } //$id == $_SESSION[ 'id' ]
?><a id="aprofile" href="friends.php?type=added&sort_by=random&id=<?php
        echo $id;
?>">Сортировать рандомно</a>
<a id="aprofile" href="friends.php?type=added&sort_by=date1&id=<?php
        echo $id;
?>">Сортировать по дате регистрации (увеличение)</a>
<a id="aprofile" href="friends.php?type=added&sort_by=date2&id=<?php
        echo $id;
?>">Сортировать по дате регистрации (убывание)</a>
</div>
</div>
</div>
<div>
...
<?php
      } //empty( $_GET[ 'type' ] ) || $_GET[ 'type' ] == "added"
    elseif ( $_GET[ 'type' ] == "incoming" )
      {
        include 'exec/datefn.php';
        include 'exec/header.php';
        include 'exec/leftmenu.php';
        if ( $id != $_SESSION[ 'id' ] )
          {
            echo "Ошибка доступа (" . $id . "," . $_SESSION[ 'id' ] . ")";
            exit( );
          } //$id != $_SESSION[ 'id' ]
        if ( $_GET[ 'sort_by' ] == "date1" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id2` = '" . $id . "'" );
          } //$_GET[ 'sort_by' ] == "date1"
        elseif ( $_GET[ 'sort_by' ] == "date2" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id2` = '" . $id . "' ORDER BY id DESC" );
          } //$_GET[ 'sort_by' ] == "date2"
        elseif ( empty( $_GET[ 'sort_by' ] ) || $_GET[ 'sort_by' ] == "random" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id2` = '" . $id . "' ORDER BY RAND()" );
          } //empty( $_GET[ 'sort_by' ] ) || $_GET[ 'sort_by' ] == "random"
        $qs->execute();
?>
<div id="content-infoname"><b>Входящие заявки</b><div style="float:right;"><a href="search.php?type=users&sort_by=random">Поиск пользователей (Добавить в друзья)</a></div></div>
<div style="min-width:0;width:415px;min-height:300px;float:left;margin-top:-10px;border-right:#BEBEBE solid 1px;">
<br>
<?php
        while ( $fr = $qs->fetch() )
          {
            ...
            if ( $user[ 'verify' ] == "1" )
              {
                $ver = ' <img src="img/verify_silver.png" width="12" height="12" style="margin-left:0;margin-right:0;margin_bottom:-2;">';
              } //$user[ 'verify' ] == "1"
            elseif ( $user[ 'verify' ] == "5" )
              {
                $ver = ' <img src="img/verify_green.png" width="12" height="12" style="margin-left:0;margin-right:0;margin_bottom:-2;">';
              } //$user[ 'verify' ] == "5"
            elseif ( $user[ 'verify' ] == "3" )
              {
                $ver = ' <img src="img/verify_blue.png" width="12" height="12" style="margin-left:0;margin-right:0;margin_bottom:-2;">';
              } //$user[ 'verify' ] == "3"
            elseif ( $user[ 'verify' ] == "4" )
              {
                $ver = ' <img src="img/verify_orange.png" width="13" height="12" style="margin-left:0;margin-right:0;margin_bottom:-2;">';
              } //$user[ 'verify' ] == "4"
            else
              {
                $ver = '';
...
<div id="content-wall-post-text">
<br><a href="add_friend.php?id=' . $user[ 'id' ] . '" id="button" style="clear:both;">Добавить в друзья</a><br><br>
</div>
</div>
</div>
</td>
...
<br><br>';
        ...
        if ( $user[ 'id' ] == "" )
          {
            echo '<div style="margin:0 -10px;padding:55px;"><center><img src="img/error.png"><br><br><b style="font-size:25px;">У Вас нет заявок</b><br><text style="font-size:14px;">Попробуйте найти себе друзей и повторить попытку</text></center></div>';
          } //$user[ 'id' ] == ""
?>
</div>
<div style="float:right;width:200px;margin-top:-8px;">
<div style="margin:10px;">
<a id="aprofile" href="friends.php?type=added&sort_by=random">Ваши друзья</a>
<a id="aprofile" href="friends.php?type=outcoming&sort_by=random">Исходящие заявки</a>
<hr style="margin-left:-14px;margin-right:-20px;margin-top:10px;margin-bottom:10px;">
<a id="aprofile" href="friends.php?type=incoming&sort_by=random">Сортировать рандомно</a>
<a id="aprofile" href="friends.php?type=incoming&sort_by=date1">Сортировать по дате регистрации (увеличение)</a>
<a id="aprofile" href="friends.php?type=incoming&sort_by=date2">Сортировать по дате регистрации (убывание)</a>
</div>
</div>
</div>
<div>
    ...
    elseif ( $_GET[ 'type' ] == "outcoming" )
      {
        ...
        if ( $_GET[ 'sort_by' ] == "date1" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id1` = '" . $id . "'" );
          } //$_GET[ 'sort_by' ] == "date1"
        elseif ( $_GET[ 'sort_by' ] == "date2" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id1` = '" . $id . "' ORDER BY id DESC" );
          } //$_GET[ 'sort_by' ] == "date2"
        elseif ( empty( $_GET[ 'sort_by' ] ) || $_GET[ 'sort_by' ] == "random" )
          {
            $qs = $dbh1->prepare( "SELECT * FROM `subs` WHERE `id1` = '" . $id . "' ORDER BY RAND()" );
          } //empty( $_GET[ 'sort_by' ] ) || $_GET[ 'sort_by' ] == "random"
        $qs->execute();
        if ( $id != $_SESSION[ 'id' ] )
          {
...
?>
<div id="content-infoname"><b>Исходящие заявки</b><div style="float:right;"><a href="search.php?type=users&sort_by=random">Поиск пользователей (Добавить в друзья)</a></div></div>
<div style="min-width:0;width:415px;min-height:300px;float:left;margin-top:-10px;border-right:#BEBEBE solid 1px;">
<br>
<?php
        while ( $fr = $qs->fetch() )
...
<div id="content-wall-post-text">
<br><a href="del_friend_sub.php?id=' . $user[ 'id' ] . '" id="button" style="clear:both;">Отменить заявку</a><br><br>
</div>
</div>
</div>
</td>
...
<br><br>';
        ...
        if ( $user[ 'id' ] == "" )
          {
            echo '<div style="margin:0 -10px;padding:55px;"><center><img src="img/error.png"><br><br><b style="font-size:25px;">Здесь ничего нет...</b><br><text style="font-size:14px;">Пхе, Вам не придётся убираться из подписчиков бывшего друга/подруги. Радуйтесь.</text></center></div>';
          } //$user[ 'id' ] == ""
?>
</div>
<div style="float:right;width:200px;margin-top:-8px;">
<div style="margin:10px;">
<a id="aprofile" href="friends.php?type=incoming&sort_by=random">Входящие заявки</a>
<a id="aprofile" href="friends.php?type=added&sort_by=random">Ваши друзья</a>
<hr style="margin-left:-14px;margin-right:-20px;margin-top:10px;margin-bottom:10px;">
<a id="aprofile" href="friends.php?type=outcoming&sort_by=random">Сортировать рандомно</a>
<a id="aprofile" href="friends.php?type=outcoming&sort_by=date1">Сортировать по дате регистрации (увеличение)</a>
<a id="aprofile" href="friends.php?type=outcoming&sort_by=date2">Сортировать по дате регистрации (убывание)</a>
</div>
</div>
</div>
<div>
