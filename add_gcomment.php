...
if ( $_SESSION[ 'loginin' ] == '1' )
  {
    if ( $_POST[ 'text' ] == NULL )
      {
        echo '<meta charset="utf-8">Проверьте, вы вообще что-то писали на поле?<meta http-equiv="refresh" content="3;gpost' . $_POST[ 'id' ] . '">';
        exit( );
      } //$_POST[ 'text' ] == NULL
    if ( $_POST[ 'text' ] == ' ' )
      {
    ...
    $q  = "INSERT INTO `gcomments` (`id`, `iduser`, `idpost`, `text`, `date`) VALUES (NULL, '" . $id . "', '" . $idpost . "', '" . $text . "', '" . time() . "')"; // добавляем коммент 
    $q1 = $dbh1->prepare( $q ); // отправляем запрос серверу
    $q1->execute();
    $q1->fetch();
    header( 'Location: gpost' . $_POST[ 'id' ] );
  } //$_SESSION[ 'loginin' ] == '1'
