...
echo $album[ 'name' ];
?></b><?php
if ( $_SESSION[ 'id' ] == $userrr[ 'id' ] )
  {
?><text style="font-size: 8pt; color: #aaa; float: right;"><a href="photoalbum_add.php?id=<?php
    echo $id;
?>">Добавить фотографию</a></text><?php
  } //$_SESSION[ 'id' ] == $userrr[ 'id' ]
?></div>
<br>
...
while ( $photo = $qs->fetch() )
  {
    ...
    $qg->execute();
    $photoo = $qg->fetch();
    echo '<a href="watchi.php?id=' . $photoo[ 'id' ] . '"><img src="imagesmall.php?image=' . $photoo[ 'image' ] . '" style="margin-right: 4px;margin-bottom: 4px;" id="albphotomin"></a>';
  } //$photo = $qs->fetch()
?>
	<br>
</div>
